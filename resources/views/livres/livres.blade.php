...
<table class="mt-5 mb-5 table table-success table-striped-columns">
  ...
  @foreach($livres as $parcour)
  
    <tr>
      <td>{{$parcour->id}}</td>
      <td>{{$parcour->titre}}</td>
      <td>{{$parcour->auteur}}</td>
      <td>{{$parcour->date}}</td>
      <td>{{$parcour->edition}}</td>

      <td>
                        <a href="{{ route('livres.edit', $parcour->id) }}" class="btn btn-sm btn-primary">Edit</a>
                        <a href="{{ route('livres.show', $parcour->id) }}" class="btn btn-sm btn-primary">Details</a>

                        
                        <form action="{{ route('livres.destroy', $parcour->id) }}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
